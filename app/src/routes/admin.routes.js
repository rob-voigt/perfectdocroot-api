...
router.get('/runs', requireApiKey, async (req, res, next) => {
  try {
    ...
    const stepsRows = Array.isArray(steps) && steps.length
      ? steps.map((step) => `
          <tr>
            ...
            <td style="padding:8px; border-bottom:1px solid #e5e7eb;">${escapeHtml(step.input_hash || '')}</td>
            <td style="padding:8px; border-bottom:1px solid #e5e7eb;">${escapeHtml(step.output_hash || '')}</td>
            <td style="padding:8px; border-bottom:1px solid #e5e7eb;">${escapeHtml(String(step.created_at ?? ''))}</td>
          </tr>
        `).join('')
      : `
          <tr>
    ...
    const artifactRows = Array.isArray(artifacts) && artifacts.length
      ? artifacts.map((a) => `
          <tr>
            ...
            <td style="padding:8px; border-bottom:1px solid #e5e7eb;">${escapeHtml(a.content_hash || '')}</td>
